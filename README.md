...
- Steps for deployment:
  ...
  | Need to type 'yes' or 'no' when asked to play again | When asked to play again, if i type in anything other than 'yes' or 'no' an error message will appear saying invalid input and the game will keep looping this until a 'yes' or 'no' is typed. | In turn typed in random lettera and words, then 'yes' and then ' no. | When random letters and words typed in, error message is show and asked to type in 'yes' or 'no'. This repeats until 'yes' or 'no' is typed in. | Pass | 
